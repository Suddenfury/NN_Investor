...
HORIZON  = 3      # predict close H trading days ahead
...
# =========================
def predict_from_today(df_supervised: pd.DataFrame, model, feature_cols: list[str], target_col: str, horizon: int, lookback: int, symbol):
    """
    Uses the MOST RECENT row in df_supervised as the anchor (features end).
    Predicts target_col at +horizon trading steps (already encoded in y during training).
    Prints last 2 weeks (lookback trading days) and the prediction.
    """
    if len(df_supervised) == 0:
        raise ValueError("df_supervised is empty (not enough data after feature engineering).")

    # Anchor = last available supervised row (meaning: we have full lookback window and y exists historically)
    # BUT: for a true 'future from today', you want the latest day with full features even if y is unknown.
    # So we rebuild a feature-only "today row" from the raw signals inside df_supervised columns.
    #
    # Easiest: construct today features directly from the ORIGINAL df_supervised columns:
    # - df_supervised already contains lag features and also 'close' etc.
    # - For today prediction, we want the latest date where all lag features exist.
    #
    # We'll take the latest row where ALL feature_cols are non-null, regardless of y.
    today_row = df_supervised.dropna(subset=feature_cols).iloc[-1]
    ...
    # Infer predicted target date via trading-step index shift
    idx = df_supervised.index
    try:
    ...
    # Print last two weeks (LOOKBACK trading days) of closes ending at anchor
    hist_slice = df_supervised.loc[:anchor_date].tail(lookback)
    ...
    if target_date is not None:
        ...
        print(f"Target date (estimated)   : +{horizon} trading steps (date not in index window)")
...
def predict(SYMBOL):
    ...
    pred = predict_from_today(data, model, feature_cols, target_col, HORIZON, LOOKBACK, symbol)
    return pred
